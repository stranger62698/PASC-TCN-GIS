...
export function SiteFooter(){return <footer className="site-footer"><div><Link className="site-brand invert" href="/"><img src="/insar-satellite-v2.png" alt=""/><span><b>LANJIFYW</b><small>城市形变智能分析</small></span></Link><p>从看见形变量，到理解形变过程，再到形成可追溯的辅助解读。</p></div><div><b>开始分析</b><Link href="/map?demo=haikou">体验公开示例</Link><Link href="/map?intent=upload">上传我的数据</Link><Link href="/datasets">管理私人数据</Link></div><div><b>了解项目</b><Link href="/showcase">案例展示</Link><Link href="/platform">平台能力</Link><Link href="/about">关于项目</Link></div><div><b>地图数据</b><span>© OpenStreetMap contributors</span><span>Tiles © Esri</span><span>天地图 · 需配置 API Key</span></div></footer>}

function MotionEnhancer(){useEffect(()=>{if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const targets=Array.from(document.querySelectorAll<HTMLElement>(".section-heading,.industry-card,.platform-preview,.case-feature,.workflow article,.case-list>a,.info-grid article,.detail-grid article,.statistics-layout>article,.metric-strip article"));targets.forEach((element,index)=>{element.classList.add("reveal-ready");element.style.setProperty("--reveal-delay",`${Math.min(index%5,4)*55}ms`)});const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("is-revealed");observer.unobserve(entry.target)}}),{threshold:.12,rootMargin:"0px 0px -5%"});targets.forEach(element=>observer.observe(element));const hoverCards=Array.from(document.querySelectorAll<HTMLElement>(".industry-card,.case-list>a,.info-grid article,.metric-strip article,.product-capability article,.case-metrics span,.architecture-flow span,.schema-grid article,.report-metrics article,.selection-summary>div"));const cleanups=hoverCards.map(card=>{card.classList.add("spotlight-card");const move=(event:PointerEvent)=>{const rect=card.getBoundingClientRect();card.style.setProperty("--pointer-x",`${((event.clientX-rect.left)/rect.width)*100}%`);card.style.setProperty("--pointer-y",`${((event.clientY-rect.top)/rect.height)*100}%`)};card.addEventListener("pointermove",move);return()=>card.removeEventListener("pointermove",move)});return()=>{observer.disconnect();cleanups.forEach(fn=>fn())}},[]);return null}
